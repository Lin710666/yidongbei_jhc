...
function createTTS({ url = DEFAULT_URL, cacheDir, defaultVoice = DEFAULT_VOICE, timeout = SYNTH_TIMEOUT, gpu } = {}) {
  ...
  async function synthesize(opts = {}) {
    ...
    // 显存独占，优先级 audio。
    //
    // 为什么 TTS 也要排队：它是**另一个进程**（Qwen TTS WebUI），但用的是同一张卡。
    // 它和 Whisper / TripoSR 撞车时，本机 8GB 显存会被瞬间吃光，而报错指向
    // "语音服务返回 500" —— 看起来像 TTS 自己坏了，其实是被别人挤掉的。
    //
    // 注意 models()（列模型）之类的轻调用**不**走这个闸门：它们不加载权重，
    // 拉进队列只会让界面状态灯无谓地延迟。
    const data = gpu && gpu.withExclusive
      ? await gpu.withExclusive({ name: '语音合成', priority: gpu.PRIORITY.audio },
        () => api(pathname, { method: 'POST', body: payload, timeoutMs: timeout }))
      : await api(pathname, { method: 'POST', body: payload, timeoutMs: timeout });
