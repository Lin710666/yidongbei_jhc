...
/** 非流式对话，返回完整文本 */
async function chat({ model, messages, temperature = 0.7, numCtx = CHAT_NUM_CTX, numPredict = 2048, timeout = DEFAULT_TIMEOUT, format, keepAlive, think }) {
 ...
 */
async function* chatStream({ model, messages, temperature = 0.7, numCtx = CHAT_NUM_CTX, numPredict = 2048, timeout = DEFAULT_TIMEOUT, signal, think }) {
  if (!model) throw mkError('未指定对话模型', 'NO_MODEL');
