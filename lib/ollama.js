 ...
 * 统一的对话上下文长度。
 *
 * 为什么必须「所有调用点用同一个值」——这是本机实测踩出来的一条硬约束：
 *   Ollama 是按 (模型, num_ctx) 分别维护一个 llama-server 进程的。
 *   只要 num_ctx 变了，它就把旧进程杀掉、重新加载一遍权重再起一个新的。
 *   在 server.log 里能直接看到 msg="llama-server started in 2.26 seconds"。
 *
 * 改之前各个调用点的值是散的：角色卡 小文/小柚=8192、阿杭=16384，
 * 营销文案=8192，填表 intake=4096。实测结果：
 *     同一张卡连着问：首字 0.08~0.11 秒
 *     换一张卡再问：  首字 2.6~2.9 秒   ← 多出来的全花在重建进程上
 * 用户体感就是「打字聊天很慢」，但单看某一项指标又都正常。
 *
 * 统一成 16384 是因为阿杭的行程方案 prompt 实测有 6234 token（8192 装不下）。
 * 代价：加载后显存从 5.0GB 涨到 5.5GB；收益：换角色卡不再重载。
 * 想改就设环境变量 WENLV_NUM_CTX —— 但注意所有调用点要一起改，否则又对不上了。
 */
const CHAT_NUM_CTX = Number(process.env.WENLV_NUM_CTX || 16384);
...
/**
 * 当前常驻显存的模型（GET /api/ps）。
 *
 * 为什么要这个：Ollama 默认把模型在显存里留 KEEP_ALIVE（本项目设的 30 分钟）。
 * 而本机 8GB 卡上，它和 Whisper / TripoSR / 深度图 / TTS 是**互相抢**的关系。
 * 所以需要先问清楚"现在显存里躺着谁"，才能决定要不要请它出去。
 *
 * 探测失败一律当成"没有常驻模型"返回空数组 —— 拿不到信息时**不该拦住**任何业务，
 * 大不了就是少省一点显存。
 */
async function loaded({ timeout = 4000 } = {}) {
  try {
    const res = await fetch(`${OLLAMA_URL}/api/ps`, { signal: AbortSignal.timeout(timeout) });
    if (!res.ok) return [];
    const data = await res.json().catch(() => ({}));
    return (data.models || []).map(m => ({
      name: m.name || m.model,
...
/**
 * 把某个模型从显存里请出去（keep_alive: 0）。
 *
 * Ollama 的约定：`/api/generate` 带 `keep_alive: 0` 表示"这次用完立刻卸载"，
 * 传空 prompt 时它只做卸载、不真的生成。返回后显存通常**不是立刻**就空，
 * 所以调用方要给一点缓冲时间再启动吃显存的任务（见 lib/gpu.js）。
 */
async function unload(model, { timeout = 15000 } = {}) {
...
/**
 * 把模型重新加载进显存（预热），让"让位"之后的下一次提问不用等冷启动。
 *
 * keep_alive 传 -1 表示"永久保留直到显式卸载"，这里不这么做 —— 用一个正常的
 * 保活时间即可，免得预热本身又变成常占。
 */
async function preload(model, { keepAlive = KEEP_ALIVE, timeout = 120000 } = {}) {
  if (!model) return { ok: false, error: '缺少模型名' };
  try {
    const res = await fetch(`${OLLAMA_URL}/api/generate`, {
      method: 'POST',
...
/**
 * 去掉 qwen3 / deepseek-reasoner 这类"思考型"模型输出的思维链段落。
 * 不清掉的话，思维链会直接糊到用户界面上，看起来像答非所问。
 *
 * 结束标记有两种，必须都认：
 *   · `</think>` / `</thinking>`  —— 各家 OpenAI 兼容接口上常见
 *   · `<｜end▁of▁thinking｜>`       —— qwen3 在 Ollama 上实际吐的是这个
 *     （注意那条竖线是全角字符 ｜，不是 ASCII 的 |）
 *
 * 这里以前只写了第一种，而下面流式路径的状态机只认第二种 ——
 * 两条路径对不上，表现为"流式时干净、非流式时思维链糊在正文前面"。
 * 是本项目的外部接入测试在假端点上回放 qwen3 的真实输出格式时暴露出来的。
 */
function stripThinking(text) {
  return String(text || '')
    // ① 完整的思考块（两种结束标记都认）
    .replace(/<think(?:ing)?>[\s\S]*?(?:<\/think(?:ing)?>|<｜end▁of▁thinking｜>)/gi, '')
    // ② 只有开标记、没有结束标记（思维链被 num_predict 截断在半路）。
    //    刻意不加 ^ 锚点：被截断的思维链常常出现在正文之后。
 ...
 * 规范化工具调用。
 *
 * Ollama 与 OpenAI 在这里有个容易踩的差别：
 *   Ollama 的 arguments 是**对象**（{query:'杭州'}）
 *   OpenAI 的 arguments 是**JSON 字符串**（'{"query":"杭州"}'）
 * 统一成 { id, name, arguments(对象) }，上层就不用到处判类型、
 * 也不会因为 "参数里有引号" 这种小事在两条链路上表现不一致。
 */
...
/** 非流式对话，返回完整文本；带 tools 时还可能返回 toolCalls */
async function chat({ model, messages, temperature = 0.7, numCtx = CHAT_NUM_CTX, numPredict = 2048, timeout = DEFAULT_TIMEOUT, format, keepAlive, think, tools }) {
  ...
  const toolCalls = normalizeToolCalls(data.message && data.message.tool_calls);
  // 关键：模型决定调工具时，content 本来就是空的。
  // 这里要是照旧抛 EMPTY，工具调用就永远走不通 —— 而且报错信息会完全指向错误的方向。
  if (!content && !toolCalls) throw mkError('模型返回为空，请重试', 'EMPTY');
  return { content, raw, model, toolCalls };
}
 ...
 */
async function* chatStream({ model, messages, temperature = 0.7, numCtx = CHAT_NUM_CTX, numPredict = 2048, timeout = DEFAULT_TIMEOUT, signal, think, tools }) {
  if (!model) throw mkError('未指定对话模型', 'NO_MODEL');
  ...
  let inThink = false;
  // Ollama 的工具调用可能分几块到，也可能一块给全，所以边收边攒
  let toolRaw = [];
    ...
    for await (const chunk of res.body) {
      ...
      for (const line of lines) {
        ...
        if (obj.message && Array.isArray(obj.message.tool_calls) && obj.message.tool_calls.length) {
          toolRaw = toolRaw.concat(obj.message.tool_calls);
        }
        ...
        if (obj.done) {
          clearTimeout(timer);
          yield { done: true, content: stripThinking(full), model, toolCalls: normalizeToolCalls(toolRaw) };
          return;
        }
      }
    }
  } catch (e) {
    // 超时是在**流读到一半**时触发的，跟上面那个 try 包住的初始 fetch 是两回事。
    // 原来这里只有 finally 没有 catch，于是 AbortError 会原样冒出去，
    // 用户在界面上看到的是「This operation was aborted」——完全不知道发生了什么。
    // 现在转成和初始连接超时同一句话。
    if (e.name === 'TimeoutError' || e.name === 'AbortError') {
      const got = stripThinking(full).length;
      throw mkError(
        `本地模型超过 ${Math.round(timeout / 1000)} 秒仍未生成完。\n`
        + (got ? `已经写出来约 ${got} 字，后面卡住了。\n` : '')
        + '常见原因：模型没完全装进显存，或者被别的程序抢了算力'
        + '（舞台上的 Live2D / 粒子背景 / 词云动画都会抢）。\n'
        + '可以先把舞台动画关掉再试一次；确实需要更久就设 OLLAMA_TIMEOUT_MS 放宽超时。',
        'TIMEOUT',
      );
    }
    throw e;
...
async function vision({ model, prompt, imagesBase64, temperature = 0.3, timeout = DEFAULT_TIMEOUT }) {
  ...
  return chat({
    model,
    temperature,
    // 视觉模型这里刻意保持 8192，不跟对话的 CHAT_NUM_CTX 走：
    // 视觉是另一个模型（如 qwen2.5vl:3b），Ollama 本来就为它单独开一个 runner，
    // 不存在和对话互相顶掉的问题；而给 3B 的视觉模型开 16384 会白白多吃显存，
    // 8GB 卡上两个模型同时驻留时很危险。只有"对话模型和视觉模型是同一个"时才会
    // 重新出现 num_ctx 对不上的情况，那种情况下把它也改成 CHAT_NUM_CTX 即可。
    numCtx: 8192,
    numPredict: 1024,
    timeout,
    messages: [{
