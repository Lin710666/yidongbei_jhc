...
const BUILTIN_CARDS = [
  {
    ...
    // 默认形象用 Live2D 官方示例「桃瀬ひより」：这批模型里画面最干净、最亲切的一套，
    // 10 个动作、口型同步，适合做讲解向导。
    // 内置的「西湖船娘」仍在形象列表里可选 —— 它是程序化绘制的，主题最贴西湖，
    // 但画质比不上专业插画模型，所以不做默认（详见 README 的"西湖形象"一节）。
    // 默认形象是内置的「江南古风少女」（西湖船娘）：它是程序化绘制的，
    // 主题就是江南水乡，且不需要先下载任何模型文件 —— 首次打开就能看到人。
    // 想要 Live2D 的写实插画质感，可在「外观 → 更换形象」里换成官方示例模型
    // （hiyori / haru 等），或换成你自己的国风模型。
    live2d: { kind: 'lake', model: 'lake-boatwoman', scale: 1, x: 0, y: 0, expression: '', idleMotion: true },
...
function createCards({ dir, file = 'cards.json' }) {
  ...
  function normalize(input, base = {}) {
    ...
    return {
      ...
      live2d: {
        // kind 决定用哪套渲染器：'lake'（内置西湖船娘）/ 'live2d' / '3d'。
        // 之前没有这个字段，前端只能靠"模型名能不能在 Live2D 列表里找到"去猜，
        // 猜不到就落到第一个 Live2D 模型上 —— 用户选了西湖船娘，界面却显示别的角色。
        kind: (c.live2d && c.live2d.kind) || 'lake',
        model: (c.live2d && c.live2d.model) || 'lake-boatwoman',
