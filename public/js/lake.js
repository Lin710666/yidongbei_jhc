...
(function () {
  ...
  /** 江南古风少女的内置形象条目（供 /api/status 与前端形象列表使用） */
  const LAKE_AVATAR = {
    kind: 'lake',
    id: 'lake-boatwoman',
    label: '江南古风少女',
    url: 'builtin:lake-boatwoman',
    // 预览图：由 test/verify-cdp.js 那一套 CDP 截图生成（零依赖，用系统 Edge）
    preview: '/avatars/jiangnan-girl.png',
    note: '程序化绘制的原创形象，专为江南题材设计：双环髻与步摇、额前花钿、'
      + '交领右衽月白上襦、青碧缠枝长裙、青花油纸伞，配色取自江南烟雨与青花瓷。'
      + '不含任何第三方模型素材，可放心用于对外展示；矢量绘制，任意分辨率都不糊。'
      + '若要写实插画质感，请换用 Live2D 类形象或上传你自己的国风模型。',
    tags: ['内置', '江南古风', '原创程序化'],
  };

  window.WenlvLake = { LakeAvatar, drawLakeScene, LAKE_AVATAR, COLORS: C };
})();
