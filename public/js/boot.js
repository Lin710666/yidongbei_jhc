...
(function () {
   ...
   */
  const TEMPLATES = [
    { id: 'video', name: '视频主页', hint: '整屏播西湖宣传片，两个功能入口' },
  ...
  /**
   * 两个功能入口（原本文旅 / 设置 / API 接入三张，API 那张按需求删了）。
   *
   ...
   * （历史）三个入口时有两个都跳到外观页，用户点完「设置」再点「API 接入」会以为没反应。
  ...
  const ENTRIES = [
    ...
    /* ★ 这里原来是第三张卡「🔌 API 接入」（跳到外观页的模型接入 + 对外开放）。
       按需求删掉了 —— 它只是一个跳转入口，真正的设置在「外观」页里还在，
       想看的时候自己进去即可，不必在开屏占一格。
       （删的是 ENTRIES 里的一项，卡片是 forEach 生成的，会自动变两张。） */
  ];
