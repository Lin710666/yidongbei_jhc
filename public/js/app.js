...
(function () {
  ...
  async function boot() {
    ...
    if (!S.history.length) {
      ...
      // 最后一批才真正去生成。
      setTimeout(() => { try { startBubbleGuide(); } catch { /* 忽略 */ } }, 1800);
    }
  ...
  async function runPlanWith(overrides) {
    if (S.busy) { toast('正在生成，请稍等或点停止', 'err'); return; }
    const params = collectPlanParams(overrides);
    appendMsg('user', `帮我排一个方案：${params.city} · ${params.days} 天`);
    const bd = appendMsg('assistant', `正在按 HikiTravel 的规划链路生成…（${params.city} · ${params.days} 天）`);
    try {
      await generate('plan', params);
      // 结果渲染在文旅页里，这里给一个能跳过去的词条 ——
      // 用户要的就是"文旅功能里那种点击跳转"
      bd.innerHTML = renderMarkdown(`已经排好了：**${params.city} · ${params.days} 天**。`);
      bd.appendChild(chipRow([
        { label: '查看完整方案 ›', value: 'tools' },
      ], () => { gotoPlanPane(); }));
    } catch (e) {
  ...
  function startBubbleGuide() {
    ...
    window.WenlvAsk.start(function (answers) {
      // 气泡里给的是中文选项，collectPlanParams 需要的是它那套键；runPlanWith 会合并
      runPlanWith(answers || {});
    });
  }
  ...
  function bindVideos() {
     ...
     * ====================================================================*/
    {
      ...
      if (side && !document.querySelector('#btn-chat-expand')) {
        ...
        applyTall(stored === '1');
        btn.addEventListener('click', () => applyTall(!document.body.classList.contains('chat-tall')));
      }
    }
  ...
  function bindComposer() {
    ...
    // 「让 AI 先问我」：同样走**分批气泡**（和进入时的引导是同一条路），
    // 不再用原来那套"一问一答、答完就生成"的老流程。
